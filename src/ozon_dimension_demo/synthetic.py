"""Упрощённая генерация облаков точек для демонстрации геометрии."""
...
def generate_two_sensor_scene(
    ...
    Это не физическая модель лазерного профилометра: здесь не моделируются
    отдельные лазерные линии, временная развёртка, энкодер и сложные окклюзии.
    Генератор нужен только для воспроизводимой проверки обработки облака точек.
    """
